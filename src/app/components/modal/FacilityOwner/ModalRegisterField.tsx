...
const ModalRegisterField = (props: ModalRegisterFieldProps) => {
  ...
  const handleOk = async () => {
    try {
      ...
      console.log("✅ validateFields xong:", values);
      console.log("🧩 infoLogin hiện tại:", infoLogin);
      if (!infoLogin?.userId) {
      ...
      const data = {
        ...
        price: Number(values.fixed || 0),
        fixedPrice: values.notfixed || "",
        openTime: values.time?.split("-")[0]?.trim() || "",   // tách giờ mở
        closeTime: values.time?.split("-")[1]?.trim() || "",  // tách giờ đóng
        openDays: values.day || "",
        link: values.link || "",
        courtDetails: values.courtDetails || "0",
      ...
      // gọi Redux action
      console.log("👉 Chuẩn bị gọi actionCreateField với data:", data);
