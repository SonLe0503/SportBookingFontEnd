...
const ReportAdmin = () => {
  ...
  return (
    <>
        ...
        <div className="m-2 rounded-[15px] flex bg-[#FFFFFF] shadow-[0_3px_10px_rgb(0,0,0,0.2)]">
          <div className="rounded-t-[15px] w-full">
            <div className="flex bg-gray-100 rounded-t-[15px]">
              ...
              <div className="w-[15%] p-2 flex justify-center text-[14px] text-gray-700">
                Tên sân
              </div>
            ...
            {data.map((user) => (
              <div key={user.id} className="flex items-center w-full">
                <div
                  className="w-[15%] p-2 flex text-[14px] text-gray-700 truncate relative cursor-pointer group justify-center"
                  title={user.name}
                >
                  {user.name}
                </div>
                <div
                  className="w-[15%] p-2 flex text-[14px] text-gray-700 truncate relative cursor-pointer group justify-center"
                  title={user.nameFacility}
                >
                  {user.nameFacility}
                </div>
                <div
                  className="w-[15%] p-2 flex truncate relative text-[14px] text-gray-700 cursor-pointer group justify-center"
      ...
      <ModalReport openModal={openModal} setOpenModal={setOpenModal}/>
    </>
  );
};
